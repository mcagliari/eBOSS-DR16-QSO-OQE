...
@everywhere begin
    ...
    R = get_rotation(GC, len_compression, cPB) #get rotation

    Pk_mocks = get_Pmocks(GC, p_weight) 
    Bk_mocks = cPB ? get_Bmocks(GC, SN) : get_Bmocks(GC, len_compression)

    if !cPB
        if SN #subtract SN from mocks
            SNr_mocks = get_rotate_SN(GC, R)
            Bk_mocks .-= SNr_mocks
        end
    end
    ...
    PcB_mocks = vcat(Pk_mocks, Bk_mocks) #concatenate mocks
    if cPB
        PcB_mocks = R * PcB_mocks #rotate cPB mocks #CHECK THIS
    end
    
    Σ = cov(transpose(PcB_mocks)) #compute covariance
    len_k = length(Σ[:,1])
    ...
    if cPB
        ...
        PpB_data = R * PpB_data #compress data
    else
        Bk_data = R * Bk_data #compress data
        PpB_data = vcat(Pk_data, Bk_data)
    end
    ...
    #model
    if cPB
        data_model = cPpB_qso_convolved_IC(PpB_recast, p, iΓ, kₚP, PkP_model, αkP_model, B0_terms, BGIC_terms, fzP_model, fzB_model, QₗP, W₀kP, k_start, WricP, WricB, PsnB, R, true)
    else
        data_model = PpcB_qso_convolved_IC(PpB_recast, p, iΓ, kₚP, PkP_model, αkP_model, B0_terms, BGIC_terms, fzP_model, fzB_model, QₗP, W₀kP, k_start, WricP, WricB, PsnB, R, true)
    end
end
...
if p == p_weight #optimal weights for P
    output_folder = get_output_foldercB(GC, p, len_compression, "data-SNsub", true, NN_weights)
else #FKP weights for P
    output_folder = get_output_foldercB(GC, p, p_weight, len_compression, "data-SNsub", true, NN_weights)
end
