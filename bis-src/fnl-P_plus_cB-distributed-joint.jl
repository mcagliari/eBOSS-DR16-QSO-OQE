...
include("fnl-P_plus_cB-model.jl")
...
@everywhere begin
    #info run
    #joint
    p = 1.6 #or 1.6 #p for model
    p_weight = 1.6 #weights used for P(k)
    ...
    #rotation and compression of data + covariance computation
    #covariance and cutting data if required

    RN = get_rotation(GC, len_compressionN, cPB) #get rotation

    Pk_mocksN = get_Pmocks(GC, p_weight) 
    Bk_mocksN = cPB ? get_Bmocks(GC, SN) : get_Bmocks(GC, len_compressionN)

    if !cPB
        if SN #subtract SN from mocks
            SNr_mocksN = get_rotate_SN(GC, RN)
            Bk_mocksN .-= SNr_mocksN
        end
    end
    ...
    PcB_mocksN = vcat(Pk_mocksN, Bk_mocksN) #concatenate mocks
    if cPB
        PcB_mocksN = RN * PcB_mocksN #rotate cPB mocks #CHECK THIS
    end

    ΣN = cov(transpose(PcB_mocksN)) #compute covariance
    len_kN = length(ΣN[:,1])
    ...
    if cPB
        ...
        PpB_dataN = RN * PpB_dataN #compress data
    else
        Bk_dataN = RN * Bk_dataN #compress data
        PpB_dataN = vcat(Pk_dataN, Bk_dataN)
    end
    ...
    RS = get_rotation(GC, len_compressionS, cPB) #get rotation

    Pk_mocksS = get_Pmocks(GC, p_weight) 
    Bk_mocksS = cPB ? get_Bmocks(GC, SN) : get_Bmocks(GC, len_compressionS)

    if !cPB
        if SN #subtract SN from mocks
            SNr_mocksS = get_rotate_SN(GC, RS)
            Bk_mocksS .-= SNr_mocksS
        end
    end
    ...
    PcB_mocksS = vcat(Pk_mocksS, Bk_mocksS) #concatenate mocks
    if cPB
        PcB_mocksS = RS * PcB_mocksS
    end

    ΣS = cov(transpose(PcB_mocksS)) #compute covariance
    len_kS = length(ΣS[:,1])
    ...
    if cPB
        ...
        PpB_dataS = RS * PpB_dataS #compress data
    else
        Bk_dataS = RS * Bk_dataS #compress data
        PpB_dataS = vcat(Pk_dataS, Bk_dataS)
    end
    ...
    #model
    if cPB
        data_model = cPpB_qso_convolved_IC(PpB_recastN, PpB_recastS, p, iΓN, iΓS, kₚPN, kₚPS, PkP_modelN, PkP_modelS, αkP_modelN, αkP_modelS, B0_termsN, B0_termsS, BGIC_termsN, BGIC_termsS, fzP_modelN, fzP_modelS, fzB_modelN, fzB_modelS, QₗPN, QₗPS, W₀kPN, W₀kPS, k_startN, k_startS, WricPN, WricPS, WricBN, WricBS, PsnBN, PsnBS, RN, RS, true)
    else
        data_model = PpcB_qso_convolved_IC(PpB_recastN, PpB_recastS, p, iΓN, iΓS, kₚPN, kₚPS, PkP_modelN, PkP_modelS, αkP_modelN, αkP_modelS, B0_termsN, B0_termsS, BGIC_termsN, BGIC_termsS, fzP_modelN, fzP_modelS, fzB_modelN, fzB_modelS, QₗPN, QₗPS, W₀kPN, W₀kPS, k_startN, k_startS, WricPN, WricPS, WricBN, WricBS, PsnBN, PsnBS, RN, RS, true)
    end
end
...
if p == p_weight #optimal weights for P
    output_folder = get_output_foldercB("joint", p, len_compressionN, len_compressionS, "data-SNsub", true, NN_weights)
else #FKP weights for P
    output_folder = get_output_foldercB("joint", p, p_weight, len_compressionN, len_compressionS, "data-SNsub", true, NN_weights)
end
