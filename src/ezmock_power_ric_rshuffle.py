...
def main(ns):
    ...
    for box_num in range(ns.start, ns.stop, ns.step):
        ...
        urlpath = f'https://data.sdss.org/sas/dr17/eboss/lss/EZmocks/v1_0_0/complete/eBOSS_QSO/'
        dataname = f'dat/EZmock_complete_eBOSS_QSO_{ns.sample}GC_{box_num:04d}.dat.fits.gz'
        randname = f'ran.shuf/EZmock_complete_eBOSS_QSO_{ns.sample}GC_{box_num:04d}.ran.shuf.fits.gz'
        ...
        rand_path = os.path.join(eboss.data_dir, 'ezmock', randname[9:-3])
        ...
        # load the data
        data = eboss.read_ezmock_complete_data(box_num, ns.sample)
        data = eboss.trim_redshift_range(data, zmin=zmin, zmax=zmax)
        
        eboss.finalize_ezmock_complete(data, eboss.ezmock_cosmo, P0_FKP=ns.P0_FKP)
        
        # load the randoms
        randoms = eboss.read_new_ezmock_complete_randoms(box_num, ns.sample, shuffle=True)
        randoms = eboss.trim_redshift_range(randoms, zmin=zmin, zmax=zmax)
        
        eboss.finalize_ezmock_complete(randoms, eboss.ezmock_cosmo, P0_FKP=ns.P0_FKP)
        ...
        if ns.p == -1.:
            ...
            eboss.save_ezmock_spectra(result, box_num, ns.sample, eztype='shuffle', **meta)
...
if __name__ == '__main__':
    ...
    group.add_argument('--p', type=float, help=h,
                       choices=[-1., 0., 1., 1.6, 3.0], required=True)
